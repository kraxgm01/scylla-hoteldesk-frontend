...
import { Users, UserCheck, UserX, Star, Search, Plus, Grid3X3, List, Calendar, Loader2 } from "lucide-react"
...
export default function GuestsPage() {
  ...
  const handleGuestUpdate = (guestId: string, updates: any) => {
    setGuests((prev) =>
      prev.map((guest) =>
        guest._id === guestId ? { ...guest, ...updates, updatedAt: new Date().toISOString() } : guest,
      ),
    )
    toast("Guest Updated", {
  ...
  const handleRoomAssignment = async (guestId: string, roomId: string) => {
    try {
      ...
      setGuests((prev) =>
        prev.map((guest) =>
          guest._id === guestId ? response.data : guest
        )
      )
      toast("Room Assigned", {
  ...
  const filteredGuests = guests.filter((guest) => {
    ...
    const matchesStatus = statusFilter === "all" || (statusFilter === "active" ? guest.isActive : !guest.isActive)

    const matchesRoom =
      roomFilter === "all" ||
      (roomFilter === "assigned" ? guest.room : roomFilter === "unassigned" ? !guest.room : true)
      ...
      <div className="flex-1 space-y-6 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">{stats.checkedInGuests}</div>
              <p className="text-xs text-muted-foreground">With assigned rooms</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Awaiting room assignment</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Total across all guests</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight">Guests Management</h2>
            <p className="text-muted-foreground">Monitor and manage all hotel guests</p>
          </div>
